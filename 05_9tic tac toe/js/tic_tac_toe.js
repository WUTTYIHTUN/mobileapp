    ...
    if (sessionStorage.getItem("tic_tac_toe_access")) {
        switch (index) {
            ...
            case 1:
                ...
                break;
            case 2:
                sessionStorage.setItem("tic_tac_toe_access", "3");  // これも修正 - "2"から"3"に
                level_1.classList.add("level-non-selected");
                level_2.classList.add("level-non-selected");
...
function gameOver(status) {
    ...
    //display game new button : display
...
//New Game Button,if you click Game will start at first
    ...
    lineArray.some(function (line) {
        ...
        if (bearTurnEnd === "1") {
            line.some(function (square) {
                if (square.classList.contains("js-clickable")) {
                    isSelect(square); // 修正：関数呼び出し時に条件を追加
                    return true;
                }
            })
            return true;
